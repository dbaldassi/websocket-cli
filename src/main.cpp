...
template<typename T>
int run_client(std::string_view host, int port, const std::string& subprotocol)
{
...
int main(int argc, char* argv[])
{
    ...
    client->callback([&]() { 
        if(secure) ret = run_client<WebSocketSecure>(host, (*port_opt ? port : WebSocketSecure::DEFAULT_PORT), subprotocol);
        else       ret = run_client<WebSocket>(host, (*port_opt ? port : WebSocket::DEFAULT_PORT), subprotocol);
    });
